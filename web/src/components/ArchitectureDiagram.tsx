 ...
 * 5-channel hybrid retrieval → blend & rerank → dialog manager → reply.
 *
 * Pure SVG with Tailwind classes on the foreignObject text so it matches
 * the rest of the site. No chart library, no dependency.
 */
  ...
  return (
    <div className="rounded-2xl border border-stone-200 bg-white shadow-card p-3 overflow-x-auto">
      ...
      >
        ...
        <rect width="1180" height="760" fill="url(#dots)" />
        ...
        <Lane
          ...
          w={1140}
          h={140}
          label="5-CHANNEL HYBRID RETRIEVAL"
        />
        ...
        <Block
          ...
          subtitle="opener · clarif · disambig"
        />
        <Block
          x={1010}
          y={580}
      ...
      <p className="px-3 pb-2 pt-3 text-[11px] text-stone-500">
        All 30+ blocks above are real Python modules under{" "}
        <code className="text-stone-700">api/app/</code>. Channel weights were
        tuned by the ablation study (<code>api/ablate.py</code>). The LTR
        reranker only fires when lexical confidence &lt; 0.45 so median
        latency stays ~27 ms.
      </p>
    </div>
  );
