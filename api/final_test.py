...
def test_pipeline_output(s: Suite) -> None:
    """The pipeline writes its candidates lazily. We import its runner
    and check it's wired correctly, but skip the file check (it's
    gitignored and only created when you actually run the pipeline)."""
    print()
    print("  --- pipeline ---")
    runner = ROOT / "pipeline" / "pipeline" / "runner.py"
    s.check("pipeline/pipeline/runner.py exists", runner.is_file())
    cand = ROOT / "data" / "faq_candidates.jsonl"
    if cand.is_file():
        n = sum(1 for line in cand.open("r", encoding="utf-8") if line.strip())
        s.check("pipeline produced candidates", n >= 1, f"{n} candidates")
...
def test_ablation_script_runnable(s: Suite) -> None:
    """The ablation / intent eval / report files are gitignored
    because they are regenerated by their respective scripts. We
    only check the scripts themselves are importable."""
    print()
    print("  --- ablation + intent-eval scripts ---")
    s.check("api/ablate.py exists",
            (ROOT / "api" / "ablate.py").is_file())
    s.check("api/intent_eval.py exists",
            (ROOT / "api" / "intent_eval.py").is_file())
...
def main() -> int:
    ...
    test_ablation_script_runnable(s)
    test_unit_tests(s)
